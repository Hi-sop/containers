...
namespace ft
{
	...
	struct __vector_base
	{
		...
		protected:
			...
			void __copy_data
				(pointer const& __new_begin_, pointer const& 
				 __new_end, pointer const& __new_end_cap_) throw();
	...
	(const allocator_type& a) throw()
	: __begin_(NULL), __end_(NULL), __end_cap_(NULL), __a_(a) 
	{
	}
	...
	(__vector_base const& _other) throw()
	{
	...
	template <typename _T, typename _Allocator>
	void __vector_base<_T, _Allocator>::__swap_data
	(__vector_base& _other) throw() 
	{
	...
	class __vector_iterator
	{
		public:
			...
			typedef typename ft::iterator_traits<iterator_type>::
		...
		__vector_iterator(const __vector_iterator<_U>& __u,
	...
	class vector : private __vector_base<_T, _Allocator>
	{
		...
		public:
			...
			vector(_InputIterator first,
         		typename enable_if <__is_input_iterator<_InputIterator>::
				value && !__is_forward_iterator<_InputIterator>::value, 
				_InputIterator>::type last, 
				const allocator_type& Alloc = allocator_type());

			template <typename _ForwardIterator>
			vector(_ForwardIterator first,
					typename enable_if<__is_forward_iterator<_ForwardIterator>
					::value, _ForwardIterator>::type last,
					const allocator_type& Alloc = allocator_type());
			...
			{
				return ft::distance(this->__begin_, this->__end_);
			}
			...
			{
				return reference(*(this->__begin_ + n));
			}
			...
			template <typename _InputIterator>
			void assign(_InputIterator first, typename enable_if
					<__is_input_iterator<_InputIterator>::value &&
					!__is_forward_iterator<_InputIterator>::value, 
					_InputIterator>::type last);
			
			template <typename _ForwardIterator>
			void assign(_ForwardIterator first, typename enable_if
	...
	{
		std::uninitialized_fill(this->__begin_, 
				this->__begin_ + n, value_type());
		this->__end_ += n;
	}
	...
	!__is_forward_iterator<_InputIterator>::value, _InputIterator>::type last,
    	const allocator_type& Alloc)
    : __vector_base<_T, _Allocator>(size_type(), Alloc)
	{
    ...
    typename enable_if<__is_forward_iterator<_ForwardIterator>::value,
		_ForwardIterator>::type last, const allocator_type& Alloc)
	...
	{
		clear();
		this->__end_ = std::uninitialized_copy
			(other.__begin_, other.__end_, this->__begin_);
	}
	...
	template <typename _T, typename _Allocator>
	void vector<_T, _Allocator>::__reconstruct_push_back
                       ...
                       _ForwardIterator>::type last)
	{
		...
		{
			clear();
			this->__end_ = std::uninitialized_copy(first, last, 
					this->__begin_);
		}
		else 
		{
	...
	template <typename _T, typename _Allocator>
	void vector<_T, _Allocator>::pop_back() throw()
	{
		if (!empty())
			__destroy_from_end(this->__end_ - 1);
	}
                       ...
                       _ForwardIterator>::type last)
	{
		...
		if (__in_size <= 0)
			return;
		if (__in_size + size() > capacity())
			reserve(__in_size + size());
	...
	(iterator position)
	{
		...
		this->__a_.destroy(std::uninitialized_copy
				(__p + 1, this->__end_--, __p));
	...
	template <typename _T, typename _Allocator>
	inline void swap(ft::vector<_T, _Allocator>& x,
			ft::vector<_T, _Allocator>& y)
	{
		x.swap(y);
	}
